...
function mountPC(mode){
  ...
  content.innerHTML=`<div class="pc-session"><section class="pc-live-room" aria-label="Your character using the computer"><div class="pc-room-label">MY HIDEOUT <span>● LIVE</span></div><canvas id="pc-room-view" width="480" height="400" aria-label="Live view of your character seated at the PC"></canvas><p>CONNECTED FROM HOME</p></section><div class="pc-desktop"><div class="pc-system"><span>● LOCAL CONNECTION</span><span>PC LVL ${state.pcLevel} / SECTOR 7</span></div><div class="pc-layout"><section class="pc-sidebar" aria-label="Computer applications"><div class="pc-logo">S_</div><small>SECTOR OS</small><button data-pc="hack" class="${mode==='hack'?'selected':''}">⌘ Intrusion</button><button data-pc="trade" class="${mode==='trade'?'selected':''}">↗ Trade</button><div class="pc-storage"><small>COLLECTED FILES</small><strong id="pc-files">${state.information} files</strong><p>Sell your data to Cipher on BLACKNET.</p></div></section><section class="pc-app"><div class="pc-window-title"><span>${mode==='hack'?'intrusion.exe':'market.exe'}</span><span>− □</span></div><div class="pc-command">guest@sector7:~$ ${mode==='hack'?'connect --target encrypted':'open BITE/BYTE'} ▌</div><div class="pc-app-body">${app}</div></section></div><div class="pc-status"><span>⚡ <b id="pc-energy">${state.energy}/100</b></span><span id="pc-balance">${state.byteBalance} BYTE</span><span>PRIVATE SESSION</span></div></div></div>`;
...
function render(now){
  ...
  if(roomView){const roomCtx=roomView.getContext('2d');roomCtx.imageSmoothingEnabled=false;roomCtx.clearRect(0,0,480,400);roomCtx.drawImage(canvas,origin.originX+64,origin.originY+14,384,320,0,0,480,400);}
}
