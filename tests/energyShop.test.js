...
import { buyEnergyRefill, ENERGY_REFILL_COST_BYTE, ENERGY_SHOP_REASON } from '../src/hackIntegration/energyShop.js';
...
test('compra bem sucedida: cobra o preco certo, enche a energia, grava gasto no ledger', () => {
  ...
  const result = buyEnergyRefill({ energyMeter, ledger });

  assert.equal(result.success, true);
  assert.equal(result.byteSpent, ENERGY_REFILL_COST_BYTE);
  assert.equal(energyMeter.value, energyMeter.max);
  assert.equal(result.energyValue, energyMeter.max);
  assert.equal(ledger.balance, 100 - ENERGY_REFILL_COST_BYTE);
  assert.equal(ledger.entries.at(-1).type, 'spend');
  assert.equal(ledger.entries.at(-1).amount, ENERGY_REFILL_COST_BYTE);
});
...
test('recusa e nao cobra nada se a energia ja estiver cheia', () => {
  ...
  assert.equal(result.reason, ENERGY_SHOP_REASON.ENERGY_FULL);
...
test('recusa e nao cobra nada (nem gasta energia) se o saldo de BYTE for insuficiente', () => {
  ...
  const ledger = makeFundedLedger(ENERGY_REFILL_COST_BYTE - 1);

  const result = buyEnergyRefill({ energyMeter, ledger });

  assert.equal(result.success, false);
  assert.equal(result.reason, ENERGY_SHOP_REASON.NOT_ENOUGH_BYTE);
  assert.equal(result.byteSpent, 0);
  assert.equal(energyMeter.value, 30, 'a energia nao mudou numa compra recusada');
  assert.equal(ledger.balance, ENERGY_REFILL_COST_BYTE - 1, 'nada foi cobrado');
});

test('compra so recarrega o que falta, nunca passa do maximo', () => {
  const energyMeter = new EnergyMeter({ max: 100, regenPerSecond: 0 });
  energyMeter.spend(5); // 95/100
  const ledger = makeFundedLedger(ENERGY_REFILL_COST_BYTE);

  buyEnergyRefill({ energyMeter, ledger });

  assert.equal(energyMeter.value, 100);
});
